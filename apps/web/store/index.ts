...
/**
 * POST 返回前 SSE 可能已插入同一条持久化消息（同 id），与乐观更新的负 id 并存；
 * 去重时同 conversation + 正 id 只保留最后一次写入（通常含完整 attachments）。
 */
function dedupePositiveIdsKeepLast(messages: ChatMessage[]): ChatMessage[] {
  const lastIdx = new Map<string, number>()
  messages.forEach((m, i) => {
    if (m.id > 0) {
      lastIdx.set(`${m.conversation_id}:${m.id}`, i)
    }
  })
  return messages.filter((m, i) => {
    if (m.id <= 0) return true
    return lastIdx.get(`${m.conversation_id}:${m.id}`) === i
  })
}

export interface Conversation {
  id: string
  /** 用户重命名后的标题，优先于 name 展示 */
  customTitle?: string
  name?: string
  kind?: string
  source?: 'chat' | 'session'
...
// Billing/provisioning entity that can own multiple workspaces.
...
interface XClawStore {
  ...
  setAwaitingReply: (value: { waiting: boolean; conversationId?: string | null; runId?: string | null }) => void
  setChatPanelOpen: (open: boolean) => void
  markConversationRead: (conversationId: string) => void
  updateConversation: (
    conversationId: string,
    updates: Partial<Pick<Conversation, 'name' | 'customTitle' | 'updatedAt'>>,
  ) => void
...
export const useXClawStore = create<XClawStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    addChatMessage: (message) =>
      set((state) => {
        const clearPayload = {
          isAwaitingReply: false as const,
          awaitingConversationId: null as null,
          awaitingRunId: null as null,
        }

        // 同一 id 再次推送：合并 metadata（补全 phase/runId），并重新判断能否结束等待
        if (message.id > 0) {
          const idx = state.chatMessages.findIndex(
            (m) => m.id === message.id && m.conversation_id === message.conversation_id
          )
          if (idx >= 0) {
            const prev = state.chatMessages[idx]
            const prevMeta =
              typeof prev.metadata === 'object' && prev.metadata !== null && !Array.isArray(prev.metadata)
            ...
            const merged: ChatMessage = {
              ...prev,
              ...message,
              metadata: { ...prevMeta, ...nextMeta } as JsonValue,
            }
            const messages = [...state.chatMessages]
            messages[idx] = merged
            const shouldClear = shouldClearAwaitingReplyForMessage(
              {
                isAwaitingReply: state.isAwaitingReply,
                awaitingConversationId: state.awaitingConversationId,
            ...
            const conversations = state.conversations.map((conv) =>
              conv.id === merged.conversation_id
                ? { ...conv, lastMessage: merged, updatedAt: merged.created_at }
                : conv
            )
            return {
              chatMessages: dedupePositiveIdsKeepLast(messages),
              conversations,
              ...(shouldClear ? clearPayload : {}),
            }
          }
        }

        const messages = dedupePositiveIdsKeepLast([...state.chatMessages, message].slice(-500))
        const conversations = state.conversations.map((conv) =>
    ...
    replacePendingMessage: (tempId, message) =>
      set((state) => {
        // 先去掉同 id 的 SSE 抢先插入，避免与负 id 乐观项并存成两条
        let next = state.chatMessages.filter(
          (m) =>
            !(
              message.id > 0 &&
              m.id === message.id &&
        ...
        next = next.map((m) => (m.id === tempId ? { ...message, pendingStatus: 'sent' as const } : m))
        return { chatMessages: dedupePositiveIdsKeepLast(next) }
      }),
    ...
    setAwaitingReply: (value) =>
      set({
        isAwaitingReply: Boolean(value.waiting),
        awaitingConversationId: value.waiting ? value.conversationId || null : null,
        awaitingRunId: value.waiting ? value.runId || null : null,
      }),
    ...
    updateConversation: (conversationId, updates) =>
      set((state) => {
        ...
        if (updates.customTitle !== undefined) {
          const t = updates.customTitle?.trim()
          setConversationTitleOverride(conversationId, t || null)
        }
