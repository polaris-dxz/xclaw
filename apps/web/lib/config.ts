...
const configuredDataDir = process.env.XCLAW_DATA_DIR || defaultDataDir
const buildScratchRoot =
  process.env.XCLAW_BUILD_DATA_DIR ||
  path.join(os.tmpdir(), 'xclaw-build')
const resolvedDataDir = isBuildPhase
  ? path.join(buildScratchRoot, `worker-${process.pid}`)
  : configuredDataDir
const resolvedDbPath = isBuildPhase
  ? (process.env.XCLAW_BUILD_DB_PATH ||
      path.join(resolvedDataDir, 'xclaw.db'))
  : (process.env.XCLAW_DB_PATH ||
      path.join(resolvedDataDir, 'xclaw.db'))
const resolvedTokensPath = isBuildPhase
  ? (process.env.XCLAW_BUILD_TOKENS_PATH ||
      path.join(resolvedDataDir, 'xclaw-tokens.json'))
  : (process.env.XCLAW_TOKENS_PATH ||
      path.join(resolvedDataDir, 'xclaw-tokens.json'))
const homedir = os.homedir()
/** 桌面版 XClaw 嵌入式网关使用 ~/.xclaw/openclaw.json；纯 OpenClaw CLI 常用 ~/.openclaw */
const defaultOpenClawStateDir = path.join(homedir, '.openclaw')
const xclawDesktopStateDir = path.join(homedir, '.xclaw')
const xclawDesktopOpenclawJson = path.join(xclawDesktopStateDir, 'openclaw.json')
...
const openclawStateDir =
  ...
  (explicitOpenClawConfigPath
    ? path.dirname(explicitOpenClawConfigPath)
    : fs.existsSync(xclawDesktopOpenclawJson)
      ? xclawDesktopStateDir
...
const openclawWorkspaceDir =
  ...
  process.env.XCLAW_WORKSPACE_DIR ||
...
export const config = {
  ...
  gatewayPort: clampInt(Number(process.env.OPENCLAW_GATEWAY_PORT || String(defaultGatewayPort)), 1, 65535, defaultGatewayPort),
  logsDir:
