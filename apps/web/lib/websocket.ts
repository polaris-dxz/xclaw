...
export function useWebSocket() {
  ...
  const getGatewayErrorHelp = useCallback((message: string): string => {
    ...
    if (
      ...
      return 'Gateway requires device identity. Open XClaw via HTTPS (or localhost), then reconnect so WebCrypto signing can run.'
    }
  ...
  const sendConnectHandshake = useCallback(async (ws: WebSocket, nonce?: string) => {
    ...
    if (nonce && !tokenOnlyFallbackRef.current) {
      try {
        ...
        if (
          err instanceof DeviceIdentityUnsupported ||
          (err as Error)?.name === 'DeviceIdentityUnsupported'
        ) {
          log.debug('Connect handshake: skipping device identity (Ed25519 not supported in this environment)')
        } else {
  ...
  const handleGatewayFrame = useCallback((frame: GatewayFrame, ws: WebSocket) => {
    ...
    if (frame.type === 'event') {
      ...
      if (frame.event === 'tick') {
        ...
        if (msg) {
          addChatMessage(
            hydrateMessageAttachmentsFromMetadata({
              id: msg.id,
  ...
  const normalizeWebSocketUrl = useCallback((rawUrl: string): string => {
    const defaultGatewayPort = 20064
    const built = buildGatewayWebSocketUrl({
      host: rawUrl,
      port: Number(process.env.NEXT_PUBLIC_GATEWAY_PORT || String(defaultGatewayPort)),
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (attempts < maxReconnectAttempts) {
          ...
          // Avoid console.error here — Next.js dev overlay treats it as an app failure; losing the
          // gateway is an environment/connectivity issue (user should restart gateway or refresh).
          log.warn('Max reconnection attempts reached; gateway may be down — refresh or restart OpenClaw')
          addLog({
            id: `error-${Date.now()}`,
            timestamp: Date.now(),
            level: 'warn',
      ...
      // Browser fires `error` with an empty Event on many disconnects (e.g. gateway restart).
      // Actionable detail is always on `onclose` (code/reason). Logging as error would spam
      // Next.js dev overlay; use debug only.
      ws.onerror = (event) => {
        if (nonRetryableErrorRef.current) return
        log.debug(
          `WebSocket error event (benign during reconnect): ${describeWebSocketEvent(event, ws)} — see disconnect log for close code`,
        )
      }
...
// Helper functions
/** Browser WebSocket `error` events are `Event` objects — they often log as `{}` in the console. */
function describeWebSocketEvent(event: Event, ws: WebSocket): string {
  const stateNames = ['CONNECTING', 'OPEN', 'CLOSING', 'CLOSED'] as const
  const stateLabel = stateNames[ws.readyState] ?? String(ws.readyState)
  return `type=${event.type} readyState=${stateLabel} url=${ws.url}`
}
