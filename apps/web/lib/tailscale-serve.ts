...
const DEFAULT_GATEWAY_PORT = Number.parseInt(process.env.OPENCLAW_GATEWAY_PORT || '20064', 10) || 20064
 ...
 * 2. Any handler proxying to configured gateway port (port-based proxy)
...
export function detectTailscaleServe(web: Record<string, any> | null | undefined, configPath?: string): boolean {
  if (web) {
    for (const hostConfig of Object.values(web) as any[]) {
      ...
      // Also detect port-based proxy to gateway (e.g. :8443 → localhost:20064)
      for (const handler of Object.values(handlers) as any[]) {
        const proxy = (handler as any)?.Proxy || ''
        if (proxy.includes(`:${DEFAULT_GATEWAY_PORT}`)) return true
      }
    }
  }
