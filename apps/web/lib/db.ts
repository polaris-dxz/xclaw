...
const isTestMode = process.env.XCLAW_TEST_MODE === '1'
...
function migrateLegacyDatabases(targetDb: Database.Database) {
  const targetPath = config.dbPath
  const targetDir = dirname(targetPath)
  const targetName = basename(targetPath)
  const markerPath = join(targetDir, '.migrated-legacy-dbs.json')

  // Skip when:
  // - current DB is not xclaw.db
  if (targetName !== 'xclaw.db') return
  if (!fs.existsSync(targetPath)) return

  // Consider both monorepo root .data and old apps/web/.data as legacy sources.
  const cwdDataDir = join(process.cwd(), '.data')
  const sourceDirs = Array.from(new Set([targetDir, cwdDataDir]))
  const sourceCandidates = Array.from(
    new Set(
      sourceDirs.flatMap((dirPath) => {
        try {
          return fs
            .readdirSync(dirPath)
  ...
  ).filter((sourcePath) => sourcePath !== targetPath && fs.existsSync(sourcePath))
  ...
  const doneMap: Record<string, string> = (() => {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as { done?: Record<string, string> }
      return parsed.done || {}
    } catch {
      return {}
    }
  ...
  for (const sourcePath of sourceCandidates) {
    if (doneMap[sourcePath]) continue

    let migratedTables = 0
    let migratedRows = 0
    try {
      targetDb.exec(`ATTACH DATABASE '${sourcePath.replace(/'/g, "''")}' AS legacy`)
      targetDb.exec('BEGIN IMMEDIATE')

      for (const { name } of tableRows) {
        const existsInLegacy = targetDb
          .prepare("SELECT name FROM legacy.sqlite_master WHERE type='table' AND name = ?")
          .get(name) as { name?: string } | undefined
        if (!existsInLegacy?.name) continue

        const targetCols = targetDb.prepare(`PRAGMA table_info("${name}")`).all() as Array<{ name: string }>
        const legacyCols = targetDb.prepare(`PRAGMA legacy.table_info("${name}")`).all() as Array<{ name: string }>
        const legacySet = new Set(legacyCols.map((c) => c.name))
        const colNames = targetCols.map((c) => c.name).filter((c) => legacySet.has(c))
        if (colNames.length === 0) continue

        const quotedCols = colNames.map((c) => `"${c}"`).join(', ')
        const placeholders = colNames.map(() => '?').join(', ')
        const insertStmt = targetDb.prepare(`INSERT OR IGNORE INTO "${name}" (${quotedCols}) VALUES (${placeholders})`)
        const selectStmt = targetDb.prepare(`SELECT ${quotedCols} FROM legacy."${name}"`)
        const rows = selectStmt.all() as Array<Record<string, unknown>>
      ...
      doneMap[sourcePath] = new Date().toISOString()
      migratedSources += 1

      logger.info(
        { source: sourcePath, target: targetPath, tables: migratedTables, rowsSeen: migratedRows },
        'Migrated legacy database into xclaw.db',
      )
    } catch (error) {
      try {
      ...
      logger.error({ err: error, source: sourcePath, target: targetPath }, 'Legacy DB migration failed')
    }
  }
...
function seedAdminUserFromEnv(dbConn: Database.Database): void {
  ...
  const userCols = dbConn.prepare(`PRAGMA table_info(users)`).all() as Array<{ name: string }>
  const hasPasswordHash = userCols.some((c) => c.name === 'password_hash')
  if (!hasPasswordHash) return

  const hasUsableLocalUser = (
    dbConn.prepare(`
  ...
  const existingByUsername = dbConn.prepare(`
    SELECT id
    FROM users
    WHERE username = ?
    LIMIT 1
  ...
  if (existingByUsername?.id) {
    dbConn.prepare(`
      ...
      WHERE id = ?
    `).run(displayName, passwordHash, existingByUsername.id)
  } else {
    dbConn.prepare(`
      INSERT INTO users (username, display_name, password_hash, role)
      VALUES (?, ?, ?, ?)
    `).run(username, displayName, passwordHash, 'admin')
  }

  logger.info(`Seeded/repaired admin user credentials from env: ${username}`)
}
...
export function needsFirstTimeSetup(): boolean {
  try {
    ...
    const localUsable = database.prepare(`
      SELECT COUNT(*) as count
      FROM users
      WHERE COALESCE(provider, 'local') = 'local'
        AND password_hash IS NOT NULL
