...
/** Thrown when Web Crypto cannot create/use Ed25519 (e.g. some Electron webviews). */
export class DeviceIdentityUnsupported extends Error {
  override readonly name = 'DeviceIdentityUnsupported'
  constructor(message = 'Ed25519 not supported by this Web Crypto implementation') {
    super(message)
  }
}
...
async function importPrivateKey(pkcs8Bytes: Uint8Array): Promise<CryptoKey> {
  try {
    return await crypto.subtle.importKey('pkcs8', pkcs8Bytes as unknown as BufferSource, 'Ed25519', false, [
      'sign',
    ])
  } catch (err) {
    if (err instanceof Error && err.name === 'NotSupportedError') {
      throw new DeviceIdentityUnsupported()
    }
    throw err
  }
}
...
export async function getOrCreateDeviceIdentity(): Promise<DeviceIdentity> {
  ...
  if (storedId && storedPub && storedPriv) {
    try {
      ...
      if (err instanceof DeviceIdentityUnsupported) {
        throw err
      }
      if (err instanceof Error && err.name === 'NotSupportedError') {
        ed25519Supported = false
