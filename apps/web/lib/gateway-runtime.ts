...
export function registerMcAsDashboard(mcUrl: string): { registered: boolean; alreadySet: boolean } {
  ...
  try {
    ...
    // downgrade that the operator should control, not XClaw.
    origins.push(origin)
    parsed.gateway.controlUi.allowedOrigins = origins

    fs.writeFileSync(configPath, JSON.stringify(parsed, null, 2) + '\n')
    logger.info({ origin }, 'Registered XClaw origin in gateway config')
    ...
    if (err?.code === 'EROFS' || err?.code === 'EACCES' || err?.code === 'EPERM') {
      logger.warn(
        { err, configPath },
        'Gateway config is read-only — skipping XClaw origin registration. ' +
        'To enable auto-registration, mount openclaw.json with write access or ' +
        'add the XClaw origin to gateway.controlUi.allowedOrigins manually.',
      )
      return { registered: false, alreadySet: false }
    }
