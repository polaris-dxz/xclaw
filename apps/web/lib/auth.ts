...
export function authenticateUser(username: string, password: string): User | null {
  ...
  if (typeof row.password_hash !== 'string' || row.password_hash.trim() === '') {
    verifyPassword(password, DUMMY_HASH)
    try { logSecurityEvent({ event_type: 'auth_failure', severity: 'warning', source: 'auth', detail: JSON.stringify({ username, reason: 'missing_password_hash' }), workspace_id: 1, tenant_id: 1 }) } catch {}
    return null
  }
