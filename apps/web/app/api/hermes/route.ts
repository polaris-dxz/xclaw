...
const HOOK_DIR = join(HERMES_HOME, 'hooks', 'xclaw')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'install-hook') {
      ...
      logger.info('Installed XClaw hook for Hermes Agent')
    ...
    if (action === 'uninstall-hook') {
      ...
      logger.info('Uninstalled XClaw hook for Hermes Agent')
...
const HOOK_YAML = `name: xclaw
description: Reports agent telemetry to XClaw
version: "1.0"
events:
  - agent:start
  - agent:end
...
const HANDLER_PY = `"""
XClaw hook for Hermes Agent.
Reports session telemetry to the control plane /api/sessions endpoint.

Configuration (via ~/.hermes/.env or environment):
  MC_URL      - XClaw base URL (default: http://localhost:3000)
...
logger = logging.getLogger("hooks.xclaw")
