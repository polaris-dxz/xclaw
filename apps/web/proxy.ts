...
import { buildXClawCsp, buildNonceRequestHeaders } from '@/lib/csp'
...
function addSecurityHeaders(response: NextResponse, _request: NextRequest, nonce?: string): NextResponse {
  ...
  response.headers.set('Content-Security-Policy', buildXClawCsp({ nonce: effectiveNonce, googleEnabled }))
