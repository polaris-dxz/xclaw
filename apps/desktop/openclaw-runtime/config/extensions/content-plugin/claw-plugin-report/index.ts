...
import {
  ...
  reportEvent,
  reportEventAsync,
  setCommonParams as setReporterCommonParams,
  setOpenclawVersion as setReporterOpenclawVersion,
  getPluginName as getReporterPluginName,
  isInitialized as getReporterInitialized,
  destroy as destroyReporter,
  readSharedParams as readReporterSharedParams,
  syncFromSharedParams as syncReporterFromSharedParams,
} from './reporter.js';
...
export class GalileoReport {
  /** 单例实例 */
  private static instance: GalileoReport | null = null;
   ...
   */
  static getInstance(): GalileoReport {
   ...
   * @param options - 初始化选项
   */
  initReport(options: TelemetryInitOptions): void {
   ...
   * @param options - 事件参数
   */
  reportFunc(name: string, options: Record<string, any> = {}): void {
    if (!name) {
   ...
   * @param options - 事件参数
   */
  async reportFuncAsync(name: string, options: Record<string, any> = {}): Promise<void> {
    if (!name) {
      console.warn('[claw-plugin-report] 事件名称不能为空');
      return;
    }
...
export type { ReportConstType } from './constants.js';

// ==================== 类型导出 ====================

export type { TelemetryConfig, TelemetryInitOptions, Logger } from './types.js';
...
export type { QClawSharedParams } from './reporter.js';
