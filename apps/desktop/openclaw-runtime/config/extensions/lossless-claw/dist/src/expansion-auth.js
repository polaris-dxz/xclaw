...
export class ExpansionAuthManager {
     ...
     *
     * NOTE: This method should be called periodically to prevent memory leaks
     * from accumulated expired/revoked grants (e.g. via setInterval every few minutes).
     */
    cleanup() {
        const now = Date.now();
        let removed = 0;
...
export function wrapWithAuth(orchestrator, authManager) {
    return {
        async expand(grantId, request) {
            ...
            const requestedTokenCap = typeof request.tokenCap === "number" && Number.isFinite(request.tokenCap) && request.tokenCap > 0
