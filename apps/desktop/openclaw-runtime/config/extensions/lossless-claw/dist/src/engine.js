...
import { safeReport } from "../index.js";
...
export class LcmContextEngine {
    ...
    constructor(deps) {
        ...
        console.log(`[lcm] engine: constructor, db=${this.config.databasePath}, threshold=${this.config.contextThreshold}, freshTailCount=${this.config.freshTailCount}, leafChunkTokens=${this.config.leafChunkTokens}`);
    ...
    async resolveSummarize(params) {
        ...
        try {
            ...
            console.error(`[lcm] resolveSummarize failed:`, err instanceof Error ? err.message : err);
        }
    ...
    async bootstrap(params) {
        ...
        console.log(`[lcm] bootstrap: sessionId=${params.sessionId}, sessionFile=${params.sessionFile}`);
        const result = await this.withSessionQueue(params.sessionId, async () => this.conversationStore.withTransaction(async () => {
            ...
            console.log(`[lcm] bootstrap: conversationId=${conversationId}, historicalMessages=${historicalMessages.length}`);
            ...
            console.log(`[lcm] bootstrap: existingCount=${existingCount}`);
            if (existingCount === 0) {
                if (historicalMessages.length === 0) {
                ...
                await this.conversationStore.markConversationBootstrapped(conversationId);
                console.log(`[lcm] bootstrap: first-time import, inserted=${inserted.length} messages`);
                // Prune HEARTBEAT_OK turns from the freshly imported data
                if (this.config.pruneHeartbeatOk) {
                    await this.pruneHeartbeatOkTurns(conversationId);
                }
                return {
                    bootstrapped: true,
            ...
            console.log(`[lcm] bootstrap: reconcile imported=${reconcile.importedMessages}, hasOverlap=${reconcile.hasOverlap}`);
        ...
        if (this.config.pruneHeartbeatOk && result.bootstrapped === false) {
            try {
                ...
                if (conversation) {
                    await this.pruneHeartbeatOkTurns(conversation.conversationId);
                }
            }
            catch {
                // Heartbeat pruning is best-effort.
            }
        }
        // ─── 上报 lcm_bootstrap ───
        const bootstrapDuration = Date.now() - bootstrapStartTime;
        safeReport('lcm_bootstrap', {
            session_id: params.sessionId,
            imported_messages: String(result.importedMessages ?? 0),
            bootstrapped: String(result.bootstrapped),
            reason: result.reason ?? '',
            duration_ms: String(bootstrapDuration),
        });
        return result;
    }
    async ingestSingle(params) {
        ...
        const stored = toStoredMessage(message);
        console.log(`[lcm] ingestSingle: sessionId=${sessionId}, role=${stored.role}, tokens=${stored.tokenCount}, content.length=${stored.content.length}`);
        ...
        if (stored.role === "user") {
            ...
            if (intercepted) {
                console.log(`[lcm] ingestSingle: large file intercepted, fileIds=${intercepted.fileIds.join(',')}, originalTokens=${estimateTokens(stored.content)}`);
                const originalTokens = stored.tokenCount;
                ...
                // ─── 上报 lcm_large_file_intercept ───
                safeReport('lcm_large_file_intercept', {
                    session_id: sessionId,
                    file_count: String(intercepted.fileIds.length),
                    original_tokens: String(originalTokens),
                    rewritten_tokens: String(stored.tokenCount),
                    compression_ratio: originalTokens > 0 ? (stored.tokenCount / originalTokens).toFixed(3) : '0',
                });
            }
        }
    ...
    async afterTurn(params) {
        ...
        console.log(`[lcm] afterTurn: sessionId=${params.sessionId}, total_messages=${params.messages.length}, new_messages=${newMsgCount}, isHeartbeat=${params.isHeartbeat ?? false}, tokenBudget=${params.tokenBudget ?? 'none'}`);
        const ingestBatch = [];
        ...
        try {
            ...
            console.log(`[lcm] afterTurn: ingested ${ingestBatch.length} messages`);
        }
        catch (err) {
            // Continue with proactive compaction even if ingest fails.
            safeReport('lcm_error', {
                session_id: params.sessionId,
                phase: 'afterTurn_ingest',
        ...
        try {
            ...
            console.log(`[lcm] afterTurn: leafTrigger shouldCompact=${leafTrigger.shouldCompact}, rawTokensOutsideTail=${leafTrigger.rawTokensOutsideTail}, threshold=${leafTrigger.threshold}`);
    ...
    async assemble(params) {
        try {
            this.ensureMigrated();
            console.log(`[lcm] assemble: sessionId=${params.sessionId}, live_messages=${params.messages.length}, tokenBudget=${params.tokenBudget ?? 'default'}`);
            const conversation = await this.conversationStore.getConversationBySessionId(params.sessionId);
            if (!conversation) {
                console.log(`[lcm] assemble: no conversation found, returning live messages`);
                safeReport('lcm_assemble_fallback', {
                    session_id: params.sessionId,
                    reason: 'no_conversation',
                    live_message_count: String(params.messages.length),
                });
                return {
            ...
            console.log(`[lcm] assemble: contextItems=${contextItems.length}, conversationId=${conversation.conversationId}`);
            if (contextItems.length === 0) {
                console.log(`[lcm] assemble: no context items, returning live messages`);
                safeReport('lcm_assemble_fallback', {
                    session_id: params.sessionId,
            ...
            console.log(`[lcm] assemble: hasSummaryItems=${hasSummaryItems}`);
            if (!hasSummaryItems && contextItems.length < params.messages.length) {
                console.log(`[lcm] assemble: incomplete bootstrap, returning live messages`);
                safeReport('lcm_assemble_fallback', {
                    session_id: params.sessionId,
                    reason: 'incomplete_bootstrap',
                    live_message_count: String(params.messages.length),
                    context_item_count: String(contextItems.length),
                });
                return {
                    messages: params.messages,
                    estimatedTokens: 0,
            ...
            console.log(`[lcm] assemble: assembled ${assembled.messages.length} messages, estimatedTokens=${assembled.estimatedTokens}`);
            ...
            this.deps.log.warn(`[lcm] assemble: failed for session=${params.sessionId}: ${err instanceof Error ? err.message : String(err)}`);
            safeReport('lcm_assemble_fallback', {
                session_id: params.sessionId,
                reason: 'exception',
                live_message_count: String(params.messages.length),
    ...
    async compactLeafAsync(params) {
        ...
        console.log(`[lcm] compactLeaf: sessionId=${params.sessionId}, tokenBudget=${params.tokenBudget ?? 'none'}, force=${params.force ?? false}`);
        return this.withSessionQueue(params.sessionId, async () => {
            const conversation = await this.conversationStore.getConversationBySessionId(params.sessionId);
            if (!conversation) {
                return {
            ...
            this.deps.log.info(`[lcm] compactLeaf: done for session=${params.sessionId} compacted=${leafResult.actionTaken} tokensBefore=${tokensBefore} tokensAfter=${leafResult.tokensAfter}`);
            const compactResult = {
                ok: true,
            ...
            // ─── 上报 lcm_compact_result ───
            if (leafResult.actionTaken) {
                const compactDuration = Date.now() - compactStartTime;
                safeReport('lcm_compact_result', {
                    session_id: params.sessionId,
                    compacted: 'true',
                    rounds: '1',
                    tokens_before: String(tokensBefore),
                    tokens_after: String(leafResult.tokensAfter),
                    compression_ratio: tokensBefore > 0 ? (leafResult.tokensAfter / tokensBefore).toFixed(3) : '0',
                    mode: 'leaf',
                    duration_ms: String(compactDuration),
                    reason: 'leaf_trigger',
                });
            }
    ...
    async compact(params) {
        this.ensureMigrated();
        const compactStartTime = Date.now();
        console.log(`[lcm] compact: sessionId=${params.sessionId}, target=${params.compactionTarget ?? 'budget'}, tokenBudget=${params.tokenBudget ?? 'none'}, force=${params.force ?? false}`);
        return this.withSessionQueue(params.sessionId, async () => {
            ...
            console.log(`[lcm] compact: evaluate shouldCompact=${decision.shouldCompact}, currentTokens=${decision.currentTokens}, threshold=${decision.threshold}, forceCompaction=${forceCompaction}`);
            ...
            if (useSweep) {
                ...
                const sweepReason = sweepResult.actionTaken
                    ? "compacted"
                ...
                this.deps.log.info(`[lcm] compact(sweep): done for session=${params.sessionId} compacted=${sweepResult.actionTaken} reason="${sweepReason}" tokensBefore=${decision.currentTokens} tokensAfter=${sweepResult.tokensAfter}`);
                // ─── 上报 lcm_compact_result (sweep) ───
                if (sweepResult.actionTaken) {
                    ...
                    safeReport('lcm_compact_result', {
                        ...
                        compacted: 'true',
                        rounds: '1',
                        tokens_before: String(decision.currentTokens),
                        tokens_after: String(sweepResult.tokensAfter),
                        compression_ratio: decision.currentTokens > 0 ? (sweepResult.tokensAfter / decision.currentTokens).toFixed(3) : '0',
                        mode: 'sweep',
                        duration_ms: String(compactDuration),
                        reason: sweepReason,
                    });
            ...
            const convergenceReason = compactResult.success
                ? didCompact
                    ? "compacted"
                    : "already under target"
                : "could not reach target";
            this.deps.log.info(`[lcm] compact(converge): done for session=${params.sessionId} compacted=${didCompact} rounds=${compactResult.rounds} reason="${convergenceReason}" tokensBefore=${decision.currentTokens} tokensAfter=${compactResult.finalTokens}`);
            // ─── 上报 lcm_compact_result (converge) ───
            if (didCompact) {
                const compactDuration = Date.now() - compactStartTime;
                safeReport('lcm_compact_result', {
                    session_id: params.sessionId,
                    compacted: 'true',
                    rounds: String(compactResult.rounds),
                    tokens_before: String(decision.currentTokens),
                    tokens_after: String(compactResult.finalTokens),
                    compression_ratio: decision.currentTokens > 0 ? (compactResult.finalTokens / decision.currentTokens).toFixed(3) : '0',
                    mode: 'converge',
                    duration_ms: String(compactDuration),
    ...
    async prepareSubagentSpawn(params) {
        this.ensureMigrated();
        console.log(`[lcm] prepareSubagentSpawn: parent=${params.parentSessionKey}, child=${params.childSessionKey}, ttlMs=${params.ttlMs ?? 'default'}`);
    ...
    async onSubagentEnded(params) {
        console.log(`[lcm] onSubagentEnded: child=${params.childSessionKey}, reason=${params.reason}`);
        const childSessionKey = params.childSessionKey.trim();
        if (!childSessionKey) {
            return;
        }
    ...
    async pruneHeartbeatOkTurns(conversationId) {
        ...
        const deleted = this.conversationStore.deleteMessages(uniqueIds);
        console.log(`[lcm] pruneHeartbeatOk: conversationId=${conversationId}, deleted=${deleted} messages`);
        return deleted;
    }
}
// ── Heartbeat detection ─────────────────────────────────────────────────────
