...
export function safeReport(action, params = {}) {
    try {
...
function buildLegacyAuthFallbackWarning() {
    return [
...
async function resolveApiKeyFromAuthProfiles(params) {
    ...
    const secretConfig = (() => {
        if (isRecord(params.runtimeConfig)) {
            const runtimeProviders = params.runtimeConfig.secrets?.providers;
    ...
    return {
        ...
        complete: async ({ provider, model, apiKey, providerApi, authProfileId, agentDir, runtimeConfig, messages, system, maxTokens, temperature, reasoning, }) => {
            try {
                ...
                console.log(`[lcm] complete: provider=${providerId}, model=${modelId}, maxTokens=${maxTokens}, temperature=${temperature}`);
                ...
                const providerLevelConfig = (() => {
                    if (!isRecord(effectiveRuntimeConfig))
                        return {};
                    const providers = effectiveRuntimeConfig
                        .models?.providers;
                    if (!providers)
                        return {};
                ...
                const resolvedModel = isRecord(knownModel) &&
                    ...
                    ? {
                        ...
                        baseUrl: typeof knownModel.baseUrl === "string"
                            ? knownModel.baseUrl
                ...
                if (!resolvedApiKey && modelAuth) {
                    try {
                        resolvedApiKey = resolveApiKeyFromAuthResult(await modelAuth.resolveApiKeyForProvider({
                            provider: providerId,
                            cfg: api.config,
                        ...
                        console.error(`[lcm] modelAuth.resolveApiKeyForProvider FAILED:`, err instanceof Error ? err.message : err);
                    }
                }
                if (!resolvedApiKey && !modelAuth) {
                ...
                console.log(`[lcm] complete: calling completeSimple, api=${resolvedModel.api}`);
                ...
                if (!isRecord(result)) {
                    console.log(`[lcm] complete: result is not a record, returning empty`);
                    return {
                        content: [],
                ...
                console.log(`[lcm] complete: done, content blocks=${contentArray.length}`);
                return {
                    ...result,
        ...
        getApiKey: async (provider, model, options) => {
            if (modelAuth) {
                try {
                    const modelAuthKey = resolveApiKeyFromAuthResult(await modelAuth.getApiKeyForModel({
...
const lcmPlugin = {
    ...
    register(api) {
        console.log(`[lcm] register: starting plugin registration`);
        // ─── 初始化上报 ───
        try {
            reporter.initReport({
                configDir: join(PLUGIN_ROOT_DIR, 'claw-plugin-report'),
                logger: api.logger,
            });
            ...
            console.warn('[lcm] reporter init failed:', err instanceof Error ? err.message : err);
        }
        const deps = createLcmDependencies(api);
        console.log(`[lcm] register: deps created, db=${deps.config.databasePath}, enabled=${deps.config.enabled}`);
        const lcm = new LcmContextEngine(deps);
        console.log(`[lcm] register: engine created, registering context engine and tools`);
        ...
        api.logger.info(`[lcm] Plugin loaded (enabled=${deps.config.enabled}, db=${deps.config.databasePath}, threshold=${deps.config.contextThreshold})`);
    },
};
