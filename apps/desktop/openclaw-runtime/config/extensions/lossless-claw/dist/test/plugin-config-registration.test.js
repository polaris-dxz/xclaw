...
function buildApi(pluginConfig, options) {
    let factory;
    const infoLog = vi.fn();
    const warnLog = vi.fn();
    ...
    const api = {
        ...
        registerContextEngine: vi.fn((_id, nextFactory) => {
            factory = nextFactory;
        }),
        registerTool: vi.fn(),
        registerHook: vi.fn(),
...
describe("lcm plugin registration", () => {
    ...
    it("uses api.pluginConfig values during register", { timeout: 20000 }, () => {
        ...
        expect(infoLog).toHaveBeenCalledWith(`[lcm] Plugin loaded (enabled=true, db=${dbPath}, threshold=0.33)`);
    });
    it("inherits OpenClaw's default model for summarization when no LCM model override is set", () => {
        const { api, getFactory } = buildApi({
            enabled: true,
        });
        api.config = defaultModelConfig("anthropic/claude-sonnet-4-6");
        lcmPlugin.register(api);
        const factory = getFactory();
        expect(factory).toBeTypeOf("function");
        const engine = factory();
        const resolved = engine.deps?.resolveModel(undefined, undefined);
    ...
    it("keeps explicit provider hints ahead of plugin summaryProvider", () => {
        ...
        expect(factory).toBeTypeOf("function");
        const engine = factory();
        const resolved = engine.deps?.resolveModel("claude-sonnet-4-6", "anthropic");
        expect(resolved).toEqual({
            provider: "anthropic",
            model: "claude-sonnet-4-6",
        });
    ...
    it("prefers runtime.modelAuth over provider env keys when available", async () => {
        ...
        expect(factory).toBeTypeOf("function");
        const engine = factory();
        await expect(engine.deps?.getApiKey("anthropic", "claude-sonnet-4-6")).resolves.toBe("model-auth-key");
    });
    it("falls back to auth-profiles.json when runtime.modelAuth is unavailable", { timeout: 20000 }, async () => {
        ...
        writeFileSync(join(agentDir, "auth-profiles.json"), JSON.stringify({
            version: 1,
            profiles: {
        ...
        api.config = defaultModelConfig(`${provider}/claude-sonnet-4-6`);
        lcmPlugin.register(api);
        const factory = getFactory();
        expect(factory).toBeTypeOf("function");
        const engine = factory();
        await expect(engine.deps?.getApiKey(provider, "claude-sonnet-4-6")).resolves.toBe("token-from-auth-store");
    });
});
//# sourceMappingURL=plugin-config-registration.test.js.map