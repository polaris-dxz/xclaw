...
import { EXPANSION_ROUTING_THRESHOLDS, classifyExpansionTokenRisk, decideLcmExpansionRouting, detectBroadTimeRangeIndicator, detectMultiHopIndicator, estimateExpansionTokens, } from "../src/expansion-policy.js";
describe("decideLcmExpansionRouting", () => {
    it("applies the expected route-vs-delegate decision matrix", () => {
        ...
        for (const scenario of cases) {
            ...
            expect(decision.triggers[scenario.expectedTrigger], scenario.name).toBe(scenario.expectedTriggerValue);
        }
    });
    ...
    it("delegates when token risk crosses the high-risk boundary", () => {
        ...
        const capJustBelowHighRisk = Math.max(1, Math.ceil(estimatedTokens / EXPANSION_ROUTING_THRESHOLDS.highTokenRiskRatio) - 1);
        const capAtOrAboveHighRisk = Math.ceil(estimatedTokens / EXPANSION_ROUTING_THRESHOLDS.highTokenRiskRatio);
        const below = decideLcmExpansionRouting({
            intent: "query_probe",
...
describe("expansion-policy indicators", () => {
    ...
    it("detects multi-hop from traversal depth and query language", () => {
        expect(detectMultiHopIndicator({
...
//# sourceMappingURL=expansion-policy.test.js.map