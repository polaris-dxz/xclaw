...
function createMockConversationStore() {
    ...
    return {
        ...
        getConversation: vi.fn(async (id) => conversations.find((c) => c.conversationId === id) ?? null),
        getConversationBySessionId: vi.fn(async (sid) => conversations.find((c) => c.sessionId === sid) ?? null),
        getOrCreateConversation: vi.fn(async (sid, title) => {
        ...
        createMessageParts: vi.fn(async (messageId, parts) => {
        ...
        getMessages: vi.fn(async (convId, opts) => {
        ...
        getMessageById: vi.fn(async (id) => messages.find((m) => m.messageId === id) ?? null),
        getMessageParts: vi.fn(async (messageId) => messageParts
            .filter((part) => part.messageId === messageId)
            .sort((a, b) => a.ordinal - b.ordinal)),
        getMessageCount: vi.fn(async (convId) => messages.filter((m) => m.conversationId === convId).length),
        getMaxSeq: vi.fn(async (convId) => {
...
function createMockSummaryStore() {
    ...
    const summaryParents = [];
    const largeFiles = [];
    const store = {
        // ── Context items ───────────────────────────────────────────────────
        getContextItems: vi.fn(async (conversationId) => {
            return contextItems
                .filter((ci) => ci.conversationId === conversationId)
                .toSorted((a, b) => a.ordinal - b.ordinal);
        }),
        getDistinctDepthsInContext: vi.fn(async (conversationId, options) => {
        ...
        appendContextMessage: vi.fn(async (conversationId, messageId) => {
        ...
        appendContextSummary: vi.fn(async (conversationId, summaryId) => {
            const existing = contextItems.filter((ci) => ci.conversationId === conversationId);
            const maxOrdinal = existing.length > 0 ? Math.max(...existing.map((ci) => ci.ordinal)) : -1;
            contextItems.push({
                conversationId,
        ...
        getContextTokenCount: vi.fn(async (conversationId) => {
        ...
        getSummary: vi.fn(async (summaryId) => {
            return summaries.find((s) => s.summaryId === summaryId) ?? null;
        }),
        getSummariesByConversation: vi.fn(async (conversationId) => {
            return summaries
                .filter((s) => s.conversationId === conversationId)
                .toSorted((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
        }),
        // ── Lineage ─────────────────────────────────────────────────────────
        linkSummaryToMessages: vi.fn(async (summaryId, messageIds) => {
        ...
        linkSummaryToParents: vi.fn(async (summaryId, parentSummaryIds) => {
        ...
        getSummaryMessages: vi.fn(async (summaryId) => {
            return summaryMessages
                .filter((sm) => sm.summaryId === summaryId)
                .toSorted((a, b) => a.ordinal - b.ordinal)
                .map((sm) => sm.messageId);
        }),
        getSummaryParents: vi.fn(async (summaryId) => {
            const parentIds = new Set(summaryParents
                .filter((sp) => sp.summaryId === summaryId)
                .toSorted((a, b) => a.ordinal - b.ordinal)
                .map((sp) => sp.parentSummaryId));
            return summaries.filter((s) => parentIds.has(s.summaryId));
        }),
        getSummaryChildren: vi.fn(async (parentSummaryId) => {
            const childIds = new Set(summaryParents
                .filter((sp) => sp.parentSummaryId === parentSummaryId)
                .toSorted((a, b) => a.ordinal - b.ordinal)
                .map((sp) => sp.summaryId));
            return summaries.filter((s) => childIds.has(s.summaryId));
        }),
        getSummarySubtree: vi.fn(async (rootSummaryId) => {
            const root = summaries.find((summary) => summary.summaryId === rootSummaryId);
            if (!root) {
                return [];
            }
            const output = [];
            const queue = [{ summaryId: rootSummaryId, parentSummaryId: null, depthFromRoot: 0, path: "" }];
            const seen = new Set();
        ...
        // ── Search ──────────────────────────────────────────────────────────
        searchSummaries: vi.fn(async (input) => {
            const limit = input.limit ?? 50;
        ...
        // ── Large files ─────────────────────────────────────────────────────
        getLargeFile: vi.fn(async (fileId) => {
            return largeFiles.find((f) => f.fileId === fileId) ?? null;
        }),
        insertLargeFile: vi.fn(async (input) => {
            const file = {
                fileId: input.fileId,
                conversationId: input.conversationId,
        ...
        getLargeFilesByConversation: vi.fn(async (conversationId) => {
        ...
        _getMessageTokenCount: (_messageId) => 0,
        // Expose internals for assertions
 ...
 */
async function ingestMessages(convStore, sumStore, count, opts) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const role = opts?.roleFn ? opts.roleFn(i) : i % 2 === 0 ? "user" : "assistant";
 ...
 */
function wireStores(convStore, sumStore) {
    sumStore._getMessageTokenCount = (messageId) => {
...
describe("LCM integration: ingest -> assemble", () => {
    ...
    beforeEach(() => {
        ...
        assembler = new ContextAssembler(convStore, sumStore);
    });
    ...
    it("assembler includes summaries alongside messages", async () => {
        ...
        const summaryMsg = result.messages.find((m) => m.content.includes('<summary id="sum_test_001"'));
    ...
    it("degrades tool rows without toolCallId to assistant text", async () => {
        ...
        expect(extractMessageText(result.messages[0].content)).toContain("legacy tool output without call id");
    });
});
// ═════════════════════════════════════════════════════════════════════════════
...
describe("LCM integration: compaction", () => {
    ...
    it("compaction creates leaf summary from oldest messages", async () => {
        ...
        const summarize = vi.fn(async (text, aggressive) => {
    ...
    it("compactLeaf uses preceding summary context for soft leaf continuity", async () => {
        const incrementalEngine = new CompactionEngine(convStore, sumStore, {
        ...
        const summarize = vi.fn(async (_text, _aggressive, options) => {
    ...
    it("compactLeaf performs one depth-zero condensation pass when incrementalMaxDepth is one", async () => {
        ...
        expect(result.actionTaken).toBe(true);
        expect(result.condensed).toBe(false);
        const condensedSummaries = sumStore._summaries.filter((summary) => summary.kind === "condensed");
    ...
    it("compactLeaf cascades without depth limit when incrementalMaxDepth is -1 (unlimited)", async () => {
        ...
        const depthsSummarized = [];
        const summarize = vi.fn(async (_text, _aggressive, options) => {
            if (options?.depth !== undefined)
                depthsSummarized.push(options.depth);
        ...
        const condensedCalls = summarize.mock.calls.filter((_call, i) => summarize.mock.calls[i][2]?.isCondensed);
    ...
    it("compaction propagates referenced file ids into summary metadata", async () => {
        const productionTailEngine = new CompactionEngine(convStore, sumStore, {
            ...defaultCompactionConfig,
            freshTailCount: 16,
        });
    ...
    it("compaction emits one durable compaction part for a leaf-only pass", async () => {
        ...
        expect(result.actionTaken).toBe(true);
        expect(result.condensed).toBe(false);
        const compactionParts = convStore._messageParts.filter((part) => part.partType === "compaction");
        expect(compactionParts).toHaveLength(1);
        const metadata = JSON.parse(compactionParts[0].metadata ?? "{}");
        ...
        expect(metadata.tokensBefore > metadata.tokensAfter).toBe(true);
    ...
    it("compaction emits durable compaction parts for leaf and condensed passes", async () => {
        const condensedFriendlyEngine = new CompactionEngine(convStore, sumStore, {
            ...defaultCompactionConfig,
            leafMinFanout: 2,
            leafChunkTokens: 100,
            condensedTargetTokens: 10,
        ...
        const compactionMetadata = compactionParts.map((part) => JSON.parse(part.metadata ?? "{}"));
        ...
        expect(leafPart.createdSummaryIds.length).toBeGreaterThanOrEqual(1);
        expect(condensedPart.createdSummaryIds.length).toBeGreaterThanOrEqual(1);
    ...
    it("depth-aware condensation sets condensed depth to max parent depth plus one", async () => {
        const depthAwareEngine = new CompactionEngine(convStore, sumStore, {
            ...defaultCompactionConfig,
            leafMinFanout: 2,
            condensedMinFanout: 2,
            leafChunkTokens: 200,
    ...
    it("depth-aware phase 2 processes shallowest eligible depth first", async () => {
        ...
        expect(firstSourceText).toMatch(/^\[\d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC - \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC\]/);
    ...
    it("includes continuity context only when condensing depth-0 summaries", async () => {
        ...
        await sumStore.appendContextSummary(depthOneConversation.conversationId, "sum_depth_one_focus_a");
        await sumStore.appendContextSummary(depthOneConversation.conversationId, "sum_depth_one_focus_b");
        const summarizeCalls = [];
        const summarize = vi.fn(async (_text, _aggressive, options) => {
            summarizeCalls.push({ options });
        ...
        const depthOneItems = depthOneContext.filter((item) => item.itemType === "summary" &&
            (item.summaryId === "sum_depth_one_focus_a" || item.summaryId === "sum_depth_one_focus_b"));
        await depthAwareEngine.condensedPass(depthOneConversation.conversationId, depthOneItems, 1, summarize);
        ...
        await sumStore.appendContextSummary(depthZeroConversation.conversationId, "sum_depth_zero_prior");
        await sumStore.appendContextSummary(depthZeroConversation.conversationId, "sum_depth_zero_focus_a");
        await sumStore.appendContextSummary(depthZeroConversation.conversationId, "sum_depth_zero_focus_b");
        const depthZeroContext = await sumStore.getContextItems(depthZeroConversation.conversationId);
        const depthZeroItems = depthZeroContext.filter((item) => item.itemType === "summary" &&
            (item.summaryId === "sum_depth_zero_focus_a" ||
                item.summaryId === "sum_depth_zero_focus_b"));
        await depthAwareEngine.condensedPass(depthZeroConversation.conversationId, depthZeroItems, 0, summarize);
    ...
    it("compaction falls back to truncation when aggressive does not converge", async () => {
        ...
        const summarize = vi.fn(async (text, _aggressive) => {
...
describe("LCM integration: retrieval", () => {
    ...
    beforeEach(() => {
        ...
        sumStore = createMockSummaryStore();
        wireStores(convStore, sumStore);
        retrieval = new RetrievalEngine(convStore, sumStore);
    });
    ...
    it("grep searches across messages and summaries", async () => {
        ...
        await ingestMessages(convStore, sumStore, 5, {
            contentFn: (i) => i === 2 ? "This message mentions the deployment bug" : `Regular message ${i}`,
        });
    ...
    it("grep returns timestamps and orders matches by recency", async () => {
        ...
        expect(result.messages[result.messages.length - 1]?.createdAt.toISOString()).toBe(oldTime.toISOString());
        expect(result.summaries[0]?.createdAt.toISOString()).toBe(newTime.toISOString());
        expect(result.summaries[result.summaries.length - 1]?.createdAt.toISOString()).toBe(midTime.toISOString());
    });
...
describe("LCM integration: full round-trip", () => {
    ...
    beforeEach(() => {
        ...
        wireStores(convStore, sumStore);
        assembler = new ContextAssembler(convStore, sumStore);
        compactionEngine = new CompactionEngine(convStore, sumStore, {
            ...defaultCompactionConfig,
            freshTailCount: 4,
        });
        retrieval = new RetrievalEngine(convStore, sumStore);
    ...
    it("multiple compaction rounds create a summary DAG", async () => {
        ...
        expect(parents.some((p) => leafSummaries.some((l) => l.summaryId === p.parentSummaryId))).toBe(true);
    });
    ...
    it("grep finds content in both original messages and summaries after compaction", async () => {
        ...
        await ingestMessages(convStore, sumStore, 8, {
            contentFn: (i) => i === 3 ? "The flamingo module has a critical bug in production" : `Normal turn ${i}`,
        });
        const summarize = vi.fn(async (text) => {
...
//# sourceMappingURL=lcm-integration.test.js.map