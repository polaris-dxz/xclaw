...
const tempDirs = [];
function createTestConfig(databasePath) {
    return {
        enabled: true,
        databasePath,
...
function createTestDeps(config) {
    return {
        ...
        isSubagentSessionKey: (sessionKey) => sessionKey.includes(":subagent:"),
...
function estimateAssembledPayloadTokens(messages) {
    let total = 0;
...
describe("LcmContextEngine.ingest content extraction", () => {
    ...
    it("intercepts oversized <file> blocks and persists large file metadata", async () => {
        await withTempHome(async () => {
            ...
            expect(storedFile.storageUri).toContain(`.openclaw/lcm-files/${conversation.conversationId}/`);
...
describe("LcmContextEngine.bootstrap", () => {
    ...
    it("does not append JSONL when no overlapping anchor exists in LCM", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "db only user" },
        });
        await engine.ingest({
            sessionId,
            message: { role: "assistant", content: "db only assistant" },
        });
        const result = await engine.bootstrap({ sessionId, sessionFile });
...
describe("LcmContextEngine.assemble canonical path", () => {
    ...
    it("falls back when DB context clearly trails live context", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "persisted only one message" },
        });
        const liveMessages = [
    ...
    it("assembles context from DB when coverage exists", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "persisted message one" },
        });
        await engine.ingest({
            sessionId,
            message: { role: "assistant", content: "persisted message two" },
        });
        const liveMessages = [{ role: "user", content: "live turn" }];
        const result = await engine.assemble({
            sessionId,
            messages: liveMessages,
            tokenBudget: 10_000,
    ...
    it("respects token budget in assembled output", async () => {
        ...
        const result = await engine.assemble({
            sessionId,
            messages: [{ role: "user", content: "live tail marker" }],
            tokenBudget: 500,
        });
    ...
    it("falls back to live messages if assembler throws", async () => {
        ...
        const originalAssembler = engine.assembler;
        engine.assembler = {
            ...originalAssembler,
            assemble: async () => {
    ...
    it("inserts synthetic tool results when assembled tool calls have no result", async () => {
        ...
        expect(result.messages[1].toolCallId).toBe("call_2");
    });
    it("repairs OpenAI function_call transcripts without dropping reasoning blocks", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "interleaved user turn" },
        });
        await engine.ingest({
            sessionId,
            message: {
        ...
        expect(result.messages[1]?.role).toBe("toolResult");
        expect(result.messages[1].toolCallId).toBe("fc_1");
    ...
    it("omits dynamic LCM system prompt guidance when no summaries exist", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "plain context one" },
        });
        await engine.ingest({
            sessionId,
            message: { role: "assistant", content: "plain context two" },
        });
        const result = await engine.assemble({
            sessionId,
            messages: [],
        ...
        const promptAddition = result.systemPromptAddition;
...
describe("LcmContextEngine fidelity and token budget", () => {
    ...
    it("preserves structured toolResult content via message_parts and assembler", async () => {
        ...
        const assembledMessage = assembled.messages[1];
        ...
        expect(assembledMessage.content[0]?.type).toBe("tool_result");
    });
    it("preserves toolName through ingest-assemble round-trip for Gemini compatibility", async () => {
        ...
        const result = assembled.messages[1];
    ...
    it("reconstructs OpenAI reasoning and function call blocks when raw metadata is missing", async () => {
        ...
        const db = engine.getConversationStore().db;
        for (const part of [...assistantParts, ...toolResultParts]) {
            const metadata = JSON.parse(part.metadata ?? "{}");
            delete metadata.raw;
            db.prepare("UPDATE message_parts SET metadata = ? WHERE part_id = ?").run(JSON.stringify(metadata), part.partId);
        }
        const assembler = new ContextAssembler(engine.getConversationStore(), engine.getSummaryStore());
        const assembled = await assembler.assemble({
            conversationId: conversation.conversationId,
        ...
        expect(assembled.messages).toHaveLength(2);
        const assistant = assembled.messages[0];
        expect(assistant.role).toBe("assistant");
        expect(assistant.content?.map((block) => block.type)).toEqual(["reasoning", "function_call"]);
        ...
        const toolResult = assembled.messages[1];
    ...
    it("ingests completed turn batches with ingestBatch", async () => {
        ...
        const conversation = await engine.getConversationStore().getConversationBySessionId(sessionId);
        expect(conversation).not.toBeNull();
        expect(await engine.getConversationStore().getMessageCount(conversation.conversationId)).toBe(3);
        expect((await engine.getSummaryStore().getContextItems(conversation.conversationId)).length).toBe(3);
    });
    it("skips heartbeat turn batches in ingestBatch", async () => {
        ...
        const conversation = await engine.getConversationStore().getConversationBySessionId(sessionId);
        expect(conversation).not.toBeNull();
        expect(await engine.getConversationStore().getMessageCount(conversation.conversationId)).toBe(1);
        expect((await engine.getSummaryStore().getContextItems(conversation.conversationId)).length).toBe(1);
    ...
    it("afterTurn runs proactive threshold compaction when tokenBudget is provided", async () => {
        ...
        expect(compactSpy).toHaveBeenCalledWith(expect.objectContaining({
            sessionId,
            tokenBudget: 4096,
...
describe("LcmContextEngine.compact token budget plumbing", () => {
    it("fails when compact token budget is missing", async () => {
        ...
        await engine.ingest({
            sessionId,
            message: { role: "user", content: "hello compact" },
        });
        const result = await engine.compact({
            sessionId,
    ...
    it("forces one compaction round for manual compaction requests", async () => {
        ...
        await engine.ingest({
            ...
            message: { role: "user", content: "trigger manual compact" },
        });
        const result = await engine.compact({
            sessionId: "manual-compact-session",
            sessionFile: "/tmp/session.jsonl",
        ...
        expect(compactFullSweepSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("reports live overflow when a forced sweep cannot compact stored context", async () => {
        ...
        expect(result.result?.details).toEqual(expect.objectContaining({
            rounds: 0,
