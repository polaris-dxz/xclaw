...
import { createDelegatedExpansionGrant, resetDelegatedExpansionGrantsForTests, revokeDelegatedExpansionGrantForSession, } from "../src/expansion-auth.js";
...
function makeEngine(params) {
    return {
        ...
        getConversationStore: () => ({
            getConversationBySessionId: vi.fn().mockResolvedValue(typeof params.conversationId === "number"
                ? {
...
const MAIN_SESSION_RESTRICTION_ERROR = "lcm_expand is only available in sub-agent sessions. Use lcm_expand_query to ask a focused question against expanded summaries, or lcm_describe/lcm_grep for lighter lookups.";
describe("createLcmExpandTool expansion limits", () => {
    ...
    it("uses remaining grant tokenCap when tokenCap is omitted for summary expansion", async () => {
        ...
        expect(mockRetrieval.expand).toHaveBeenCalledWith(expect.objectContaining({
            summaryId: "sum_a",
            tokenCap: 120,
        }));
    });
...
//# sourceMappingURL=lcm-expand-tool.test.js.map