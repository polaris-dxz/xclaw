...
function makeExpandResult(overrides = {}) {
    return {
        children: [],
        messages: [],
        estimatedTokens: 0,
...
function makeExpansionResult(overrides = {}) {
    return {
        expansions: [],
        citedIds: [],
        totalTokens: 0,
...
describe("wrapWithAuth", () => {
    ...
    beforeEach(() => {
        ...
        mockOrchestrator = { expand: mockExpand };
    });
    ...
    it("throws when grant is invalid", async () => {
        ...
        await expect(authorized.expand("grant_fake", request)).rejects.toThrow(/authorization failed.*not found/i);
    ...
    it("throws when grant is expired", async () => {
        ...
        await expect(authorized.expand(grant.grantId, request)).rejects.toThrow(/authorization failed.*expired/i);
        expect(mockExpand).not.toHaveBeenCalled();
    });
    it("throws when grant is revoked", async () => {
        ...
        await expect(authorized.expand(grant.grantId, request)).rejects.toThrow(/authorization failed.*revoked/i);
    });
    ...
    it("fails when grant token budget is exhausted across calls", async () => {
        ...
        await expect(authorized.expand(grant.grantId, {
            summaryIds: ["sum_b"],
            conversationId: 1,
            tokenCap: 50,
...
describe("ExpansionOrchestrator", () => {
    ...
    beforeEach(() => {
        ...
        orchestrator = new ExpansionOrchestrator(mockRetrieval);
    });
    it("expands multiple summaryIds and collects citedIds", async () => {
        mockRetrieval.expand
            .mockResolvedValueOnce(makeExpandResult({
            children: [
    ...
    it("handles expansion with messages", async () => {
        mockRetrieval.expand.mockResolvedValue(makeExpandResult({
            messages: [
    ...
    it("describeAndExpand passes conversationId through to expand", async () => {
        ...
        expect(mockRetrieval.grep).toHaveBeenCalledWith(expect.objectContaining({ conversationId: 42 }));
    });
    it("describeAndExpand biases expansion order toward newer summaries", async () => {
        ...
        mockRetrieval.expand.mockImplementation(async (input) => {
    ...
    it("describeAndExpand allows query mode without conversationId", async () => {
        ...
        expect(mockRetrieval.grep).toHaveBeenCalledWith(expect.objectContaining({ conversationId: undefined }));
    });
});
// ===========================================================================
...
//# sourceMappingURL=expansion-auth.test.js.map