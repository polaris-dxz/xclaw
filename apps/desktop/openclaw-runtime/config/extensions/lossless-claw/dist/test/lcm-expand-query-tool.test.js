...
import { createDelegatedExpansionGrant, resolveDelegatedExpansionGrantId, resetDelegatedExpansionGrantsForTests, } from "../src/expansion-auth.js";
import { getDelegatedExpansionContextForTests, getExpansionDelegationTelemetrySnapshotForTests, resetExpansionDelegationGuardForTests, stampDelegatedExpansionContext, } from "../src/tools/lcm-expansion-recursion-guard.js";
...
function makeDeps(overrides) {
    return {
        ...
        complete: vi.fn(),
        callGateway: (params) => callGatewayMock(params),
...
function makeEngine(params) {
    return {
        ...
        getConversationStore: () => ({
            getConversationBySessionId: vi.fn(async () => typeof params.conversationId === "number"
                ? {
...
describe("createLcmExpandQueryTool", () => {
    ...
    it("returns timeout when delegated run exceeds 120 seconds", async () => {
        ...
        const methods = callGatewayMock.mock.calls.map(([opts]) => opts.method);
    ...
    it("greps summaries first when query is provided", async () => {
        ...
        expect(retrieval.grep).toHaveBeenCalledWith(expect.objectContaining({
            query: "deploy regression",
            mode: "full_text",
            scope: "summaries",
            conversationId: 7,
    ...
    it("blocks delegated re-entry with deterministic recursion errors", async () => {
        ...
        expect(first.details.error).toContain("Recovery: In delegated sub-agent sessions, call `lcm_expand` directly");
        expect(first.details.error).toContain("Do NOT call `lcm_expand_query` from delegated context.");
...
//# sourceMappingURL=lcm-expand-query-tool.test.js.map