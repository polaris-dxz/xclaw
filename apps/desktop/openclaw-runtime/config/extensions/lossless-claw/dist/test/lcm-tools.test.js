...
import { createDelegatedExpansionGrant, resetDelegatedExpansionGrantsForTests, } from "../src/expansion-auth.js";
...
function buildLcmEngine(params) {
    return {
        ...
        getConversationStore: () => ({
            getConversationBySessionId: vi.fn(async () => params.conversationId == null
                ? null
                : {
...
describe("LCM tools session scoping", () => {
    ...
    it("lcm_expand query mode infers conversationId from delegated grant", async () => {
        ...
        const tool = createLcmExpandTool({
            deps: makeDeps(),
            lcm: buildLcmEngine({ retrieval }),
            sessionId: "agent:main:subagent:session-1",
        });
        const result = await tool.execute("call-1", { query: "recent snippet" });
        expect(retrieval.grep).toHaveBeenCalledWith(expect.objectContaining({ conversationId: 42, query: "recent snippet" }));
        expect(result.details.expansionCount).toBe(1);
    });
    it("lcm_grep forwards since/before and includes local timestamps in text output", async () => {
        ...
        const tool = createLcmGrepTool({
            deps: makeDeps(),
            lcm: buildLcmEngine({ retrieval, conversationId: 42 }),
            sessionId: "session-1",
        });
        ...
        expect(result.content[0].text).toContain(expectedTimestamp);
    });
    it("lcm_describe blocks cross-conversation lookup unless allConversations=true", async () => {
        ...
        expect(scoped.details.error).toContain("Not found in conversation 42");
        const cross = await tool.execute("call-4", {
            id: "sum_foreign",
            allConversations: true,
        });
        expect(cross.content[0].text).toContain("meta conv=99");
        expect(cross.content[0].text).toContain("manifest");
    });
});
//# sourceMappingURL=lcm-tools.test.js.map