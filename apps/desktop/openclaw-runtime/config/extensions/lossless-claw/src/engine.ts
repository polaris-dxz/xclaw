...
import { safeReport } from "./report.js";
...
export class LcmContextEngine implements ContextEngine {
    ...
    const result = await this.withSessionQueue(params.sessionId, async () =>
      this.conversationStore.withTransaction(async () => {
        ...
        if (existingCount === 0) {
          ...
          await this.conversationStore.markConversationBootstrapped(conversationId);
          this.deps.log.info(`[lcm] bootstrap: first-time import, inserted=${inserted.length} messages`);
        ...
        this.deps.log.info(`[lcm] bootstrap: reconcile imported=${reconcile.importedMessages}, hasOverlap=${reconcile.hasOverlap}`);
  ...
  async assemble(params: {
    ...
    try {
      ...
      this.deps.log.info(`[lcm] assemble: ${assembled.messages.length} messages, estimatedTokens=${assembled.estimatedTokens}`);
      ...
      this.deps.log.warn(
        `[lcm] assemble: failed for session=${params.sessionId}: ${err instanceof Error ? err.message : String(err)}`,
      );
      safeReport('lcm_assemble_fallback', {
        session_id: params.sessionId,
        reason: 'exception',
  ...
  async compactLeafAsync(params: {
    ...
    return this.withSessionQueue(params.sessionId, async () => {
      ...
      this.deps.log.info(
        `[lcm] compactLeaf: done for session=${params.sessionId} compacted=${leafResult.actionTaken} tokensBefore=${tokensBefore} tokensAfter=${leafResult.tokensAfter}`,
      );
  ...
  async compact(params: {
    ...
    return this.withSessionQueue(params.sessionId, async () => {
      ...
      this.deps.log.info(`[lcm] compact: evaluate shouldCompact=${decision.shouldCompact}, currentTokens=${decision.currentTokens}, threshold=${decision.threshold}, force=${forceCompaction}`);
      const targetTokens =
      ...
      if (useSweep) {
        ...
        this.deps.log.info(
          `[lcm] compact(sweep): done for session=${params.sessionId} compacted=${sweepResult.actionTaken} reason="${sweepReason}" tokensBefore=${decision.currentTokens} tokensAfter=${sweepResult.tokensAfter}`,
        );
      ...
      this.deps.log.info(
        `[lcm] compact(converge): done for session=${params.sessionId} compacted=${didCompact} rounds=${compactResult.rounds} reason="${convergenceReason}" tokensBefore=${decision.currentTokens} tokensAfter=${compactResult.finalTokens}`,
      );
