...
export function runLcmMigrations(
  ...
  try {
    ...
    console.error("[lcm] migration: backfillSummaryDepths failed:", error);
    // Continue with remaining migrations — depth backfill is non-critical
  }

  try {
    backfillSummaryMetadata(db);
  } catch (error) {
    console.error("[lcm] migration: backfillSummaryMetadata failed:", error);
    // Continue with remaining migrations — metadata backfill is non-critical
  }
