 ...
 * 触发 weixin 渠道热重载。
 *
 * 通过 bump `channels.weixin._accountsUpdatedAt` 触发 gateway 配置热重载，
 * 让新落盘的账号文件在当前进程内尽快生效，避免要求用户手动重启 gateway。
 */
export async function triggerWeixinChannelReload(): Promise<void> {
  const configPath = resolveConfigPath();
  try {
    if (!fs.existsSync(configPath)) {
      logger.warn(`triggerWeixinChannelReload: config not found path=${configPath}`);
      return;
    }
    ...
    logger.info(`triggerWeixinChannelReload: bumped channels.weixin._accountsUpdatedAt`);
  } catch (err) {
    logger.error(`triggerWeixinChannelReload: failed err=${String(err)}`);
    throw err;
  }
}
