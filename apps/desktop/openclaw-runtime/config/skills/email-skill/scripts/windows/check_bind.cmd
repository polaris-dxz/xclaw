@echo off
REM check_bind.cmd — 检查用户是否已绑定指定邮箱 (Windows CMD版)
REM
REM 用法:
REM   check_bind.cmd --email EMAIL
REM
REM 输出（stdout JSON）:
REM   成功: {"success": true, "data": {"bound": true}, "message": "Success"}
REM   失败: {"success": false, "error_code": 3, "message": "具体错误信息"}
...
REM 构造请求体并发送请求
set "REMOTE_URL=%REMOTE_BASE_URL%/data/4118/forward"
set "TMP_FILE=%TEMP%\email_skill_%RANDOM%.tmp"
set "TMP_STATUS=%TEMP%\email_skill_status_%RANDOM%.tmp"

set "BODY={\"email\": \"%EMAIL%\"}"

curl -s -o "%TMP_FILE%" -w "%%{http_code}" -X POST "%PROXY_BASE_URL%/proxy/api" -H "Remote-URL: %REMOTE_URL%" -H "Content-Type: application/json" -d "%BODY%" > "%TMP_STATUS%" 2>nul
