...
try:
    from openclaw_gateway_chat import send_chat_message as openclaw_send_chat
except ImportError:
    openclaw_send_chat = None
...
# Paths: monorepo root (parent of apps/), static UI in apps/web/
_API_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR = os.path.abspath(os.path.join(_API_DIR, "..", ".."))
...
def _resolve_openclaw_workspace():
    """Resolve OpenClaw workspace path: env OPENCLAW_WORKSPACE > ~/.openclaw/openclaw.json > default.
    openclaw.json may have agents.defaults.workspace or agents.list[].workspace (multi-workspace);
    when list exists we use the default agent's workspace or the first entry."""
    env_path = os.environ.get("OPENCLAW_WORKSPACE")
    if env_path:
        return os.path.expanduser(env_path.strip())
    config_path = os.path.join(os.path.expanduser("~"), ".openclaw", "openclaw.json")
    if not os.path.isfile(config_path):
        return os.path.join(os.path.expanduser("~"), ".openclaw", "workspace")
    try:
    ...
    # agents.list: multiple workspaces; prefer default: true, else first
    list_ = agents.get("list")
    if isinstance(list_, list) and len(list_) > 0:
        for item in list_:
            if isinstance(item, dict) and item.get("default") is True:
                w = item.get("workspace")
                if w:
                    return os.path.expanduser(str(w).strip())
        w = list_[0].get("workspace") if isinstance(list_[0], dict) else None
        if w:
            return os.path.expanduser(str(w).strip())
...
# 生图脚本：优先使用仓库内随包分发的 nano-banana-pro 脚本（与 apps/api 同一 Python 环境）
_OPENCLAW_SKILLS = os.path.join(os.path.expanduser("~"), ".openclaw", "skills")
_INREPO_NANO_SCRIPT = os.path.join(_API_DIR, "vendor", "nano-banana-pro", "generate_image.py")
if os.environ.get("STAR_OFFICE_GEMINI_SCRIPT"):
    GEMINI_SCRIPT = os.environ["STAR_OFFICE_GEMINI_SCRIPT"]
elif os.path.isfile(_INREPO_NANO_SCRIPT):
    GEMINI_SCRIPT = _INREPO_NANO_SCRIPT
else:
    GEMINI_SCRIPT = os.path.join(_OPENCLAW_SKILLS, "nano-banana-pro", "scripts", "generate_image.py")

if os.environ.get("STAR_OFFICE_GEMINI_PYTHON"):
    GEMINI_PYTHON = os.environ["STAR_OFFICE_GEMINI_PYTHON"]
elif os.path.isfile(_INREPO_NANO_SCRIPT) and os.path.isfile(GEMINI_SCRIPT) and GEMINI_SCRIPT == _INREPO_NANO_SCRIPT:
    # 与 Flask 共用 uv/venv，子进程自带 requests / pillow
    GEMINI_PYTHON = sys.executable
else:
    _gp = shutil.which("python3") or shutil.which("python")
    GEMINI_PYTHON = _gp if _gp else "python3"
# 是否使用 nano-banana-pro 的 generate_image.py（参数与输出格式与旧 gemini_image_generate 不同）
USE_NANO_BANANA_SCRIPT = os.path.basename(GEMINI_SCRIPT) == "generate_image.py"
...
def ensure_electron_standalone_snapshot():
    ...
    - browser uses apps/web/index.html
    - Electron uses apps/web/electron-standalone.html
    """
...
def _asset_category_for_path(rel_path: str) -> str:
    """Group assets into high-level categories for AI generation.

    This lets us do "apply to whole category" (e.g. all main-character sheets)
    when generating with nano-banana.
    """
    p = (rel_path or "").lower()
    # Main character sheets
    if "star-idle-v5" in p or "star_idle" in p:
        return "main_character"
    if "star-working-spritesheet-grid" in p or "star_working" in p:
        return "main_character"
    # Cats
    if "cats-spritesheet" in p or "/cats" in p:
        return "cats"
    # Fallback: generic category by top-level directory prefix
    if "/" in p:
        return p.split("/", 1)[0]
    return "misc"


def _expand_asset_category_paths(primary_rel_path: str) -> list[str]:
    """Given a primary asset path, return a list of related assets in the same category.

    The primary path is always included. For categories we know (main_character / cats)
    we search other files in FRONTEND_PATH that match the same category.
    """
    primary = (primary_rel_path or "").strip().lstrip("/")
    if not primary:
        return []
    cat = _asset_category_for_path(primary)
    results: set[str] = {primary}
    # Only eagerly expand for a few well-known character categories to avoid surprises.
    if cat in {"main_character", "cats"}:
        try:
            for p in FRONTEND_PATH.rglob("*"):
                if not p.is_file():
                    continue
                rel = p.relative_to(FRONTEND_PATH).as_posix()
                if _asset_category_for_path(rel) == cat:
                    results.add(rel)
        except Exception:
            # Best-effort; if rglob fails we still return at least primary.
            pass
...
def _generate_rpg_background_to_webp(out_webp_path: str, width: int = 1280, height: int = 720, custom_prompt: str = "", speed_mode: str = "fast"):
    ...
    When USE_NANO_BANANA_SCRIPT is True, this function delegates to the
    nano-banana-pro generate_image.py helper (OpenClaw skill) which accepts:
      --prompt / --filename / [--input-image] / [--resolution]

    Otherwise, it uses the legacy gemini-image-generate script with richer model
    fallback logic.
    ...
    if not os.path.exists(GEMINI_SCRIPT):
        raise RuntimeError("生图脚本环境缺失：Gemini 生图脚本不存在")
    ...
    # 分两种模式：
    # 1) USE_NANO_BANANA_SCRIPT=True: 调用 nano-banana-pro 的 generate_image.py
    #    输出路径通过 --filename 控制，最后一行 stdout 会包含 "Image saved: <path>"
    # 2) legacy 模式：调用兼容 gemini-image-generate 的脚本，要求返回 JSON {files:[...]}

    if USE_NANO_BANANA_SCRIPT:
        # 目标 PNG 临时路径（后续再转 WebP）
        tmp_dir = tempfile.mkdtemp(prefix="rpg-bg-nano-")
        png_path = os.path.join(tmp_dir, "generated.png")
        ...
        # nano-banana-pro 脚本约定：如果提供 input-image，会做编辑模式
        if os.path.exists(ROOM_REFERENCE_IMAGE):
            cmd.extend(["--input-image", ROOM_REFERENCE_IMAGE])
        ...
        # 脚本会打印 "Image saved: /abs/path/to.png"，但我们更信任 png_path
        gen_path = png_path
...
if not os.path.exists(JOIN_KEYS_FILE):
    if os.path.exists(os.path.join(ROOT_DIR, "config", "join-keys.sample.json")):
        try:
            with open(os.path.join(ROOT_DIR, "config", "join-keys.sample.json"), "r", encoding="utf-8") as sf:
                sample = json.load(sf)
...
def openclaw_chat():
    ...
    try:
        ...
        if not message:
            return jsonify({"ok": False, "msg": "message 不能为空"}), 400
        result = openclaw_send_chat(message)
        if result["ok"]:
            return jsonify({"ok": True, "reply": result["reply"]})
        return jsonify({"ok": False, "msg": result.get("error") or "gateway error"}), 502
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
@app.route("/assets/ai-generate", methods=["POST"])
def assets_ai_generate():
    """Use nano-banana (Gemini) to generate or edit sprite/asset images and replace files.

    Request (multipart/form-data):
      - path: required, target asset path relative to FRONTEND_PATH
      - scope: optional, "single" (default) or "category"
      - prompt: optional text prompt for the model
      - ref: optional reference image file for edit mode

    Behavior:
      - Validates target path is within FRONTEND_PATH and exists.
      - When scope == "category", expands to all assets in the same logical group
        (e.g. all main character sheets, all cat sheets).
      - For each target, calls nano-banana generate_image.py with optional input-image,
        then rescales/crops the result to match the target file's size, preserving format.
    """
    guard = _require_asset_editor_auth()
    if guard:
        return guard
    if not USE_NANO_BANANA_SCRIPT:
        return jsonify({"ok": False, "msg": "nano-banana-pro 脚本未启用，无法使用 AI 生成替换"}), 500
    try:
        rel_path = (request.form.get("path") or "").strip().lstrip("/")
        scope = (request.form.get("scope") or "single").strip().lower()
        ...
        primary_target = (FRONTEND_PATH / rel_path).resolve()
        try:
            primary_target.relative_to(FRONTEND_PATH.resolve())
        except Exception:
            return jsonify({"ok": False, "msg": "非法 path"}), 400

        if primary_target.suffix.lower() not in ASSET_ALLOWED_EXTS:
            return jsonify({"ok": False, "msg": "仅支持图片资产进行 AI 生成替换"}), 400
        if not primary_target.exists():
            return jsonify({"ok": False, "msg": "目标文件不存在"}), 404

        # Ensure Gemini API key & script env
        runtime_cfg = load_runtime_config()
        api_key = (runtime_cfg.get("gemini_api_key") or "").strip()
        if not api_key:
            return jsonify({"ok": False, "code": "MISSING_API_KEY", "msg": "Missing GEMINI_API_KEY or GOOGLE_API_KEY"}), 400
        if not (os.path.exists(GEMINI_PYTHON) and os.path.exists(GEMINI_SCRIPT)):
            return jsonify({"ok": False, "msg": "生图脚本环境缺失：generate_image.py 未配置"}), 500

        # Determine target paths set
        if scope == "category":
            rel_paths = _expand_asset_category_paths(rel_path)
        else:
            rel_paths = [rel_path]

        # Persist reference file once if provided
        ref_path = None
        tmp_dir = None
        if ref_file:
            tmp_dir = tempfile.mkdtemp(prefix="asset-ai-ref-")
            ref_path = os.path.join(tmp_dir, "ref-upload.png")
        ...
        for rp in rel_paths:
            target = (FRONTEND_PATH / rp).resolve()
            try:
                target.relative_to(FRONTEND_PATH.resolve())
            except Exception:
                errors.append({"path": rp, "msg": "非法 path"})
                continue
            if not target.exists():
                errors.append({"path": rp, "msg": "目标文件不存在"})
                continue
            if target.suffix.lower() not in ASSET_ALLOWED_EXTS:
                errors.append({"path": rp, "msg": "不支持的文件类型"})
                continue

            # Snapshot default & backup prior version
            default_snap = Path(str(target) + ".default")
            if not default_snap.exists():
                try:
                    shutil.copy2(target, default_snap)
            ...
            # Read target size
            if Image is None:
                errors.append({"path": rp, "msg": "Pillow 不可用，无法标准化尺寸"})
                continue
            try:
                with Image.open(str(target)) as im_t:
                    width, height = im_t.size
            except Exception as e:
                errors.append({"path": rp, "msg": f"读取尺寸失败: {e}"})
                continue

            # Build prompt with safety defaults
            base_prompt = (
                "Pixel-art spritesheet compatible with the existing game. "
                "Strictly preserve the existing grid layout, frame count, and transparent background. "
                "Keep character pose and anchor roughly the same, only change style, outfit, colors and face. "
            )
            full_prompt = (base_prompt + (prompt or "")).strip()

            # Generate via nano-banana-pro generate_image.py
            tmp_gen_dir = tempfile.mkdtemp(prefix="asset-ai-gen-")
            out_png = os.path.join(tmp_gen_dir, "generated.png")
            cmd = [
                GEMINI_PYTHON,
                GEMINI_SCRIPT,
                "--prompt",
            ...
            # Optional edit mode with reference
            if ref_path and os.path.exists(ref_path):
                cmd.extend(["--input-image", ref_path])
            ...
            try:
                ...
                errors.append({"path": rp, "msg": f"调用脚本失败: {e}"})
                continue

            if proc.returncode != 0:
                err_text = (proc.stderr or proc.stdout or "").strip()
                errors.append({"path": rp, "msg": f"生图失败: {err_text}"})
                continue

            if not os.path.exists(out_png):
                errors.append({"path": rp, "msg": "生图文件不存在"})
                continue

            # Resize/crop to match target size and overwrite
            try:
                with Image.open(out_png) as im_gen:
                    im_gen = im_gen.convert("RGBA")
                    if im_gen.size != (width, height):
                        im_gen = im_gen.resize((width, height), Image.Resampling.LANCZOS)
                    if target.suffix.lower() == ".webp":
                        im_gen.save(str(target), "WEBP", lossless=True, quality=100, method=6)
                    else:
                        im_gen.save(str(target), "PNG")
                st = target.stat()
                successes.append({"path": rp, "size": st.st_size})
            except Exception as e:
                errors.append({"path": rp, "msg": f"写入失败: {e}"})
            finally:
                try:
        ...
        return jsonify({"ok": True, "updated": successes, "errors": errors})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
